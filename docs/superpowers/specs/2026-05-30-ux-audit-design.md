...
**A1 — Registration form only offers CHO/CO roles** *(medium)*  
...
**A2 — No tracking of pending invites** *(medium)*  
...
**A3 — No approval notification triggers** *(high)*  
...
**A4 — Stats tiles are not filtered** *(high)*  
...
**A5 — No open/closed indicator on the camp list** *(high)*  
...
**A6 — Desktop table duplicates the City column** *(low)*  
...
**A7 — No date-relative highlighting ("today's camps")** *(medium)*  
...
**A8 — Date display bug on confirmation screen** *(critical)*  
...
**A9 — City and Chapter are free-text with no autocomplete** *(medium)*  
...
**A10 — No venue/school field** *(medium — deferred)*  
...
**A11 — No edit or delete after creation** *(medium)*  
...
**A12 — "Create another →" is near-invisible** *(low)*  
...
**A13 — No live refresh during camp** *(high)*  
...
**A14 — Child name and aspiration answers not visible in registrations** *(medium)*  
...
**A15 — No export (CSV/download)** *(medium)*  
...
**A16 — SharePanel shows all users, not just relevant city/chapter** *(low-medium)*  
...
**B1 — No notification when access is approved** *(high — same as A3)*  
COs self-register, are put in pending, and have no way to know when they've been approved without polling.

**B2 — No "check status" button on the pending page** *(medium)*  
...
**B3 — No visual distinction between "my camps" and "shared with me"** *(medium)*  
...
**B4 — No prompt to share with CHO after creating a camp** *(medium)*  
...
**B5 — Open/close requires navigating to detail page** *(medium)*  
...
**C1 — Empty state message is wrong for CHOs** *(critical)*  
...
**C2 — No guidance for CHO between approval and first camp** *(medium)*  
...
**C3 — No fullscreen QR mode for camp day use** *(high)*  
...
**C4 — Single-item bottom nav on mobile looks broken** *(low)*  
...
**D1 — Invalid session ID silently shows Welcome instead of error** *(critical)*  
...
**D2 — Camp-closed screen is a dead end** *(medium)*  
...
**D3 — City pre-filled from session, but isn't** *(high)*  
...
**D4 — Parent must re-enter all details for a second child** *(critical)*  
...
**D5 — No skip option on hard aspiration questions** *(medium)*  
...
**D6 — Image generation failure is silent** *(high)*  
...
**D7 — No recovery if parent taps "Next Child" before saving the card** *(high)*  
...
**D8 — Volunteer might rush the parent** *(medium)*  
...
**D9 — Area label is confusing alongside City** *(low)*  
...
**D10 — Download doesn't confirm where the file went** *(medium)*  
...
**D11 — Phone number normalisation on second registration** *(medium — post-camp)*  
...
**E1 — No real-time data anywhere** *(medium)*  
...
**E2 — Error messages are raw Supabase errors** *(medium)*  
...
**E3 — No toast/notification system** *(medium)*  
